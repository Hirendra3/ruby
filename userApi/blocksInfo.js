...
const getHomePageData = async (req, response, next) => {
    ...
    try {
        ...
        const [blockDataResults, transactionResults] = await Promise.all([
            ...
            new Promise(resolve => generateTransactionData(listTransactions, resolve))
        ]);
        ...
        let recentBlk = (blockDataResults.length > 0) ? blockDataResults[0].blockNumber : 0;
...
const gettransactions = () => {
  ...
  const getlastTransaction = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await transaction.find({ type: { $in: ["TransferContract", "TriggerSmartContract", "TransferAssetContract"] } })
		  .sort({ created_at: -1 })
		  .limit(100)
		  .exec();
  ...
  const gettotalAccount = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await userAddress.findOne().sort({ created_at: -1 }).countDocuments().exec();
		resolve(data);
	  } catch (error) {
		console.error(error);
		reject(error);
  ...
  const gettotalStaked = () => {
    return new Promise((resolve, reject) => {
        freezeInfo.find().exec(async (err1, data1) => {
            try {
                if (data1.length > 0) {
                    const stakedData = await freezeInfo.aggregate([
                        { "$match": { "status": "staked" } },
  ...
  const gettransactions24hours = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		...
		}).sort({ created_at: -1 }).countDocuments().exec();
		console.log("transaction",data)
		resolve(data);
	  } catch (error) {
		console.error(error);
  ...
  const getaccount24hours = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await 
			userInfo.find( {"created_at":{$gt:new Date(Date.now() - 24*60*60 * 1000)}, "status":1}).sort({created_at: -1}).countDocuments().exec();
  ...
  const getcontracts24hours = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await 
		contracts.find( {"created_at":{$gt:new Date(Date.now() - 24*60*60 * 1000)}}).sort({created_at: -1}).countDocuments().exec();
  ...
  const gettoken24hours = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await token.find( {"created_at":{$gt:new Date(Date.now() - 24*60*60 * 1000)}}).sort({created_at: -1}).countDocuments().exec();
		console.log("token",data);
		resolve(data);
	  } catch (error) {
		console.error(error);
  ...
  const getstaked24hours = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await 	freezeInfo.aggregate([
			{
			  "$match": { "status" : "staked", "staked_time": { "$gt": new Date(Date.now() - 24*60*60 * 1000) },}
			},
			{
			  "$group": {'_id': 'null','totals': {'$sum': "$amount_staked" }},
			  }
  ...
  const getblockData24hours = () => {
	return new Promise(async (resolve, reject) => {
	  try {
		const data = await 	blockInfo.find( {"created_at":{$gt:new Date(Date.now() - 24*60*60 * 1000)}}).sort({created_at: -1}).countDocuments().exec();
...
function generateBlockData(data, callback) {
	...
	if(length > 0) {
		...
		data.forEach((val) => {
			...
			transaction.find({blockNumber:val['blockNumber']}).sort({created_at: -1}).countDocuments().exec(function(err, res) {
				var blockData = {
